...
impl Constellation {
    ...
    pub(super) fn handle_matrix_thread_reset(
        &mut self,
    ...
    pub(super) fn handle_matrix_thread_init_finished(
        &mut self,
        root_id: OwnedEventId,
    ) -> Task<Action<Message>> {
        if self.active_thread_root.as_ref() == Some(&root_id) {
    ...
    pub(super) fn handle_dismiss_error(&mut self) -> Task<Action<Message>> {
        self.error = None;
        self.error_autoclose_deadline = None;
        if matches!(
            self.sync_status,
    ...
    pub(super) fn handle_jump_to_message_or_load_context(
        &mut self,
    ...
    pub(super) fn handle_toggle_members_panel(&mut self) -> Task<Action<Message>> {
        self.needs_layout_scroll_restoration = true;
        self.needs_threaded_layout_scroll_restoration = true;
    ...
    pub(super) fn handle_members_fetched(
        &mut self,
        res: Result<Vec<matrix::RoomMemberInfo>, String>,
    ) -> Task<Action<Message>> {
        ...
        match res {
            ...
            Err(e) => {
                self.set_error(
                    crate::fl!("error-failed-fetch-members", error = e.to_string()).to_string(),
                );
            }
        }
        Task::none()
    ...
    pub(super) fn handle_toggle_pinned_panel(&mut self) -> Task<Action<Message>> {
        self.needs_layout_scroll_restoration = true;
        self.needs_threaded_layout_scroll_restoration = true;
    ...
    pub(super) fn handle_pinned_events_fetched(
        &mut self,
        res: Result<Vec<matrix::PinnedEventInfo>, String>,
    ) -> Task<Action<Message>> {
