...
impl Constellation {
    pub fn handle_update(&mut self, message: Message) -> Task<Action<Message>> {
        let task = match message {
            ...
            Message::TabActivated(entity) => self.handle_tab_activated(entity),
            Message::TabClosed(entity) => self.handle_tab_closed(entity),
            Message::CloseTab(tab) => self.handle_close_tab(&tab),
            ...
            Message::ThreadInfoUpdated {
                room_id,
                root_id,
                unread,
            } => self.handle_thread_info_updated(room_id, root_id, unread),
