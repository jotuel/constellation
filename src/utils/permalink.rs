...
//! flow (CLI argument intake, the `fi.joonastuomi.constellation://` URI scheme,
...
//!   `fi.joonastuomi.constellation://open?url={any of the above, encoded}` —
...
/// The `fi.joonastuomi.constellation://` scheme prefix consumed by `main.rs`.
const APP_SCHEME: &str = "fi.joonastuomi.constellation://";
...
/// `fi.joonastuomi.constellation://open?url=...` wrapper (recursing on the
...
/// Pull the `url` query parameter out of a `fi.joonastuomi.constellation://`
...
mod tests {
    ...
    fn app_scheme_wraps_matrix_to() {
        ...
        let wrapped = "fi.joonastuomi.constellation://open?url=https%3A%2F%2Fmatrix.to%2F%23%2F%21abc%3Aexample.org%2F%24def%3Aexample.org";
        let t = ok(wrapped);
        match t {
            PermalinkTarget::Event { room, event, .. } => {
                assert_eq!(
    ...
    fn app_scheme_wraps_matrix_uri() {
        ...
        let wrapped = "fi.joonastuomi.constellation://open?url=matrix%3Au%2Falice%3Aexample.org";
        let t = ok(wrapped);
        assert_eq!(
            t,
            PermalinkTarget::User(UserId::parse("@alice:example.org").unwrap())
    ...
    fn app_scheme_missing_url_param() {
        let err = parse("fi.joonastuomi.constellation://open").unwrap_err();
        assert_eq!(err, PermalinkError::MissingUrlParam);
    }

    #[test]
    fn app_scheme_empty_url_param() {
        let err = parse("fi.joonastuomi.constellation://open?url=").unwrap_err();
        assert_eq!(err, PermalinkError::MissingUrlParam);
    }
