...
- **Stickers and custom emojis (MSC2545)** — Send and display custom emojis and standalone stickers using the MSC2545 Image Packs specification. Timeline messages render standalone stickers in borderless presentation with reactions and action controls (reply, react, download, permalink), and render custom emojis inline from homeserver media cache with jumbomoji presentation for emoji-only messages. The composer emoji picker now features dedicated "Emojis" and "Stickers" tabs, shortcode autocomplete/search, and automatic substitution of recognized `:shortcode:` into MSC2545 HTML `<img>` tags. Room Settings allows administrators to create packs, upload images, and assign shortcodes, and User Settings provides management for globally subscribed room packs (#480).
...
- **Homeserver URL previews** — Leveraged Matrix 0.19's `Media::get_media_preview()` endpoint to fetch OpenGraph previews directly via the homeserver rather than leaking client IP addresses through direct web scrapes. Rich URL preview cards display page title, description, domain/site name, and thumbnail images (including MXC and HTTP media) for links in timeline messages, and respect the "Display Media Previews" user preference (#492).
- **Search query language and autocomplete** — Use structured search filters directly from the header search bar: filter by room with `#room` or `in:room`, by sender with `@user` or `from:user`, by room type with `is:dm` or `scope:group`, and by date with `after:YYYY-MM-DD` or `before:YYYY-MM-DD`. Typing `#` or `@` opens a suggestions popover listing matching joined rooms or room members, which can be selected to complete the filter. Text search terms and phrase queries are sanitized to prevent homeserver and Tantivy parser syntax errors (#505).
...
- **Search unfilters room list on launch and distinct close icon** — Pressing Enter or clicking submit to launch a search in a tab now stops filtering the room list in the sidebar, and the close search button uses the `window-close-symbolic` icon to distinguish it from the search icon (#507).
- **OAuth 2.0 / OIDC and QR login stability** — Fixed multiple issues affecting alternative login options: resolved a startup crash when restoring saved OIDC sessions, added missing device code authorization grant and client metadata so MSC4108 QR code login succeeds with Matrix Authentication Service (MAS), prevented logouts from disabling login buttons until restart, properly revoked OAuth session tokens on logout, ensured post-login UI (spaces sidebar, window title, image packs, ignored users) updates immediately upon signing in, buffered cold-start OIDC callback URIs until engine initialization finishes, cleanly released in-progress client state on login cancellation, and surfaced actionable error messages with the authorization URL if the system browser fails to launch (#483).
### Maintenance
